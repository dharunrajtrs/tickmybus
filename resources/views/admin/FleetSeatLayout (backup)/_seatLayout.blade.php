                    ...
                    <table class="table table-hover" style="border-collapse: collapse; border-spacing: 0px; width: 100%;" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                        <thead>
                            <tr>
                                 ...
                                 <th>{{ trans('view_pages.deck_type')}}</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($results as $key => $result)
                                <tr>
                                  ...
                                  <td>{{ $result->deck_type ?? '-' }}</td>                           
                                  <td>
                                  <div class="dropdown">
                                            <button type="button" class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">@lang('view_pages.action')
                                             </button>
                                           <div class="dropdown-menu">                                          
                                                <a class="dropdown-item" href="{{url('fleet_seat_layout/edit/'.$result->fleet_id.'/deck_type/'.$result->deck_type) }}">{{ trans('view_pages.view_and_modify_seats')}}
                                                </a>                         

                                               @if (auth()->user()->can('delete_seat_layout'))
                                                <a class="dropdown-item sweet-delete"  href="#" data-url="{{url('fleet_seat_layout/delete/'.$result->fleet_id.'/deck_type/'.$result->deck_type) }}">{{ trans('view_pages.delete')}}</a>
                                                @endif
