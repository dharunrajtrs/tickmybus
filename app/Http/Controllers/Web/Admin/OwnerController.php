...
class OwnerController extends BaseController
{
    ...
    public function index()
    {
        ...
        $sub_menu = 'manage_owners';

        $activeOwners = Owner::whereApprove(true)->count();
        $inactiveOwners = Owner::whereApprove(false)->count();

        return view('admin.owners.index', compact('page', 'main_menu', 'sub_menu', 'activeOwners', 'inactiveOwners'));
    }

    public function getAllOwner(QueryFilterContract $queryFilter)
    {
    ...
    public function create()
    {
        ...
        return view('admin.owners.create', compact('page', 'main_menu', 'sub_menu','needed_document'));
    }

    public function store(StoreOwnerRequest $request){

        $created_params = $request->only(['company_name','owner_name','name','surname','mobile','phone','email','address','postal_code','city','no_of_vehicles','tax_number','bank_name','ifsc','account_no','headquarters_pincode','headquarters_address','headquarters_city']);
        ...
        if ($uploadedFile = $this->getValidatedUpload('company_logo', $request)) {
            $userParam['profile_picture'] = $this->imageUploader->file($uploadedFile)
                ->saveProfilePicture();
        }
    ...
    if($request->has('needed_document'))
    {
        foreach($request->needed_document as $key => $document){
            ...
            $docController->uploadOwnerDoc($name,$expiry_date,$request,$owner,$doc,$doc_number);
        }
    }

        $message = trans('succes_messages.owner_added_succesfully');

        return redirect("owners/by_area")->with('success', $message);
    }
    ...
    public function getById(Owner $owner)
    {
        ...
        $item = $owner;
        $needed_document = OwnerNeededDocument::active()->get();
        return view('admin.owners.update', compact('item', 'page', 'main_menu', 'sub_menu','needed_document'));
    }
    ...
    public function update(Owner $owner,UpdateOwnerRequest $request){
        ...
        $userParam = $request->only(['name','email','mobile']);
        if ($request->company_logo && $uploadedFile = $this->getValidatedUpload('company_logo', $request)) {
            $userParam['profile_picture'] = $this->imageUploader->file($uploadedFile)->saveProfilePicture();
        }
