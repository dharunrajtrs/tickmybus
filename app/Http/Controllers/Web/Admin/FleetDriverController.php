...
class FleetDriverController extends BaseController
{
    ...
    */
    public function getApprovedFleetDrivers(QueryFilterContract $queryFilter)
    {
        if (access()->hasRole(RoleSlug::SUPER_ADMIN)) {
                ...
                $this->validateAdmin();
                $query = Driver::where('approve', true)->where('owner_id', auth()->user()->owner->id)->orderBy('created_at', 'desc');
    ...
    public function getApprovalPendingFleetDrivers(QueryFilterContract $queryFilter)
    {
         ...
         if (access()->hasRole(RoleSlug::SUPER_ADMIN)) {
                ...
                // $this->validateAdmin();
                $query = Driver::where('approve', false)->where('owner_id', auth()->user()->owner->id)->orderBy('created_at', 'desc');
                // $query = Driver::orderBy('created_at', 'desc');
            }
            $results = $queryFilter->builder($query)->customFilter(new DriverFilter)->paginate();
    ...
    */
    public function create()
    {
        ...
        return view('admin.fleet-drivers.create', compact('services', 'page', 'countries', 'main_menu', 'sub_menu', 'owner'));
    }
  ...
  public function store(CreateDriverRequest $request)
    ...
    public function getById(Driver $driver)
    {
        ...
        return view('admin.fleet-drivers.update', compact('item', 'services',  'page', 'countries', 'main_menu', 'sub_menu', 'owner'));
    }
    ...
    {
        // dd($driver);
        $updatedParams = $request->only(['service_location_id', 'name','mobile','email','gender','owner_id']);
