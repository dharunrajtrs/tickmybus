...
use App\Models\Admin\CommonFleetSeatLayout;
...
class CommonSeatLayoutController extends BaseController
{
     ...
     */
    public function __construct(CommonFleetSeatLayout $seatLayout)
    {
    ...
    public function index()
    {
        ...
        return view('admin.CommonFleetSeatLayout.index', compact('page', 'main_menu', 'sub_menu'))->render();
    }
    public function create()
    {
        ...
        return view('admin.CommonFleetSeatLayout.create', compact('page', 'main_menu', 'sub_menu','companies'));
    }
    ...
    public function getAllSeatLayout(QueryFilterContract $queryFilter)
    {
        ...
        $query = CommonFleetSeatLayout::join('comman_fleets', 'Common_fleet_seat_layouts.fleet_id', '=', 'comman_fleets.id')
        ...
        return view('admin.CommonFleetSeatLayout._seatLayout', compact('results'));
    }
...
// dd($request->all());
  ...
  $fleetSeatLayout = CommonFleetSeatLayout::where('fleet_id', $fleet->id)->whereIn('deck_type',['lower','upper'])->first();
  $user_checking_id = auth()->user()->id;
        $find_owner_id = Owner::where('user_id', $user_checking_id)->first();
        $owner_id = $find_owner_id->id;
  ...
  if ($fleetSeatLayout != null)
  {
    throw ValidationException::withMessages(['license_number' => __('Layout exists for Bus')]);
      ...
      foreach ($seat_layouts as $seat_layout)
      {
          ...
          CommonFleetSeatLayout::create($seats);
      }
  ...
  return redirect('common_fleet_seat_layout')->with('success', $message);
    ...
    public function getById(CommanFleet $fleet)
    {
     ...
     return view('admin.CommonFleetSeatLayout.seat_layout_view', compact('page', 'main_menu','sub_menu','layout_type','bus', 'seatLayoutView'));
