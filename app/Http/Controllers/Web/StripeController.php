...
// dd(config('stripe.pk'));
        ...
        return view('stripe.stripe', compact('amount', 'payment_for', 'currency', 'user_id','journey_id'));
    }
...
$total1=($amount1*100);
        ...
        $session = \Stripe\Checkout\Session::create([
            ...
            'mode'        => 'payment',
            'success_url' => route('checkout.success') . '?productname=' . urlencode($productname) . '&payment_for=' . urlencode($payment_for) . '&currency=' . urlencode($currency) . '&amount=' . urlencode($amount) . '&user_id=' . urlencode($user_id). '&journey_id=' . urlencode($journey_id),
        ...
        $journey_id = $request->get('journey_id');
         ...
         if ($payment_for=="wallet") {
            ...
            $wallet_add_history_model::create([
                ...
                $journey_user = JourneyUser::where('journey_id', $journey_id)->where('user_id', $user_id)->first();

                $journey_user->update(['is_paid' => true]);

            }


        return view('success',['success'],compact('journey_id'));
    }
